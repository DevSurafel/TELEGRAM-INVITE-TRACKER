...
class InviteTrackerBot:
    ...
    async def track_new_member(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        for new_member in update.message.new_chat_members:
            try:
                ...
                # Update invite count for the inviter
    ...
    async def handle_check(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        user_id = int(query.data.split('_')[1])  # Extract user ID from the callback data

        # Fetch the user's invite count
        invite_count = self.invite_counts.get(user_id, 0)
        balance = invite_count * 50
        next_milestone = invite_count + (4 - (invite_count % 4))
        remaining = max(next_milestone - invite_count, 0)

        # Construct the message content for this specific user
        if invite_count >= 6:
            message = (
                ...
                f"👥 Invites: {invite_count} people\n"
                f"💰 Balance: {balance} ETB\n"
                f"🚀 Remaining to withdrawal: 0 people\n"
                f"-----------------------\n\n"
                f"Keep inviting to earn more rewards!"
            )
            keyboard = [
