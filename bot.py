...
class InviteTrackerBot:
    ...
    async def send_invite_info(self, update: Update, user_data: Dict[str, Any], unique_id: str):
        ...
        buttons = [
            [InlineKeyboardButton("Check", callback_data=f"check_{user_data['user_id']}"),
             InlineKeyboardButton("Key🔑", callback_data=f"key_{user_data['user_id']}")]
        ]
    ...
    async def handle_key(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if invite_count >= 200:
            if 'withdrawal_key' not in user_data:
